...
def generate_CLI(cell):
    ...
    #file_r = open("C:\\FAO\\Programs\\"+str(cell)+".TMP", 'r')
    ...
    for year in range(year_0, year_0+years):
        """
        file = open(str(cell)+'_'+str(year)+'.TMP', "w")
	...
	file.close()
	shutil.move(os.getcwd()+'/'+str(cell)+'_'+str(year)+'.TMP', 'C:\\FAO\\AquaCrop\\DATA\\'+str(cell)+'\\')
        """
        ###
    ...
    """
    file_r = open('C:\\FAO\\AquaCrop\\DATA\\'+str(cell)+'\\'+str(cell)+".PLU", 'r')
    lines=file_r.readlines()
    file_r.close()
    year_0=int(lines[4].split()[0])
    LINES=lines[9:]
    ...
    for year in range(year_0, year_0+years):
        ...
        file = open(str(cell)+'_'+str(year)+'.PLU', "w")
	...
	file.close()
	shutil.move(os.getcwd()+'/'+str(cell)+'_'+str(year)+'.PLU', 'C:\\FAO\\AquaCrop\\DATA\\'+str(cell)+'\\')
    """
